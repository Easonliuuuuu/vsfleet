...
vsfleet treats every vCenter as a named context, the way `kubectl` treats clusters
...
$ vsfleet context test customer-a
Context: customer-a
...
vsfleet replaces that with one command per question.
...
- **Failure isolation.** A dead proxy, an expired password or a rotated
  ...
  still answers.
- **Diagnostics that name the fault.** `vsfleet doctor` walks the connection one
  stage at a time and stops at the first real problem.
- **A terminal interface that opens by default.** Run `vsfleet` and it puts
...
vsfleet context list
vsfleet context test customer-a
vsfleet status                       # every context, connection state and latency
vsfleet doctor customer-a            # stage-by-stage diagnosis

vsfleet vm list --context prod
vsfleet template list --all-contexts
vsfleet datastore list --all-contexts
vsfleet host list --context prod -f esxi-07
vsfleet search ubuntu-24
vsfleet search nvme --kind datastore -o json

vsfleet                               # the same estate, interactively
vsfleet ui --all-contexts             # an explicit alias, open with every vCenter merged
```

## The terminal interface

`vsfleet` — or, spelled out, `vsfleet ui` — opens the estate rather than one
...
```
vsfleet  all 3 vCenters  ·  2 connected · 1 failed
────────────────────────────────────────────────────────────────────────────────
...
`vsfleet context add` asks for — endpoint, username, credential, route, TLS
policy — with a **Test connection** row that runs the identical stage-by-stage
diagnosis `vsfleet doctor` prints, and a **Discover from the server** action next
...
`~/.config/vsfleet/state.json` (`VSFLEET_STATE` to override), never in
...
`vsfleet context add` writes the file for you; this is what it looks like. The
location is the platform user configuration directory
(`~/.config/vsfleet/config.toml` on Linux), overridable with `--config` or
`VSFLEET_CONFIG`. It is written atomically with `0600` permissions.
...
| Reference | Meaning |
|---|---|
| `keyring:<key>` | Read from the OS secret store under service `vsfleet` |
...
```
                       cmd/vsfleet  ──►  internal/cli  ──►  internal/tui
                                            │                  │
                                            └────────┬─────────┘
                                                     │
                  ┌─────────────────────────┼─────────────────────────┐
...
|---|---|---|
| Entry experience | `vsfleet` opens the interface directly; add/edit/test/delete a context from inside it; remember the last context, tab and sort mode | **done** |
...
A new user can install vsfleet, run `vsfleet`, configure a direct or proxied
