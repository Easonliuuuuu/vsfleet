...
func (c *Collector) captureContext(parent context.Context, cc *config.Context, browseDatastores bool) ContextResult {
	...
	for _, group := range []vsphere.FetchGroup{vsphere.GroupVMs, vsphere.GroupHosts, vsphere.GroupClusters, vsphere.GroupResourcePools, vsphere.GroupDVSwitches, vsphere.GroupDatastores, vsphere.GroupNetworks} {
		...
		case vsphere.GroupVMs:
			...
			// collection above), but so coverage-gated consumers — the
			// snapshot-age health rule, diff's snapshot lifecycle tracking —
			// can tell "this run never looked at snapshots" (an import with no
			// vSnapshot worksheet) apart from "this run looked and found none".
