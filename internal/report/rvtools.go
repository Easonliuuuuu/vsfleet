...
// WriteRVTools writes the five persisted RVTools-compatible sheets plus the
...
func WriteRVTools(w io.Writer, data assessment.ExportData) error {
	...
	for _, name := range []string{"vHost", "vCluster", "vDatastore", "vSnapshot", "vsfleetCoverage"} {
...
func coverageRows(data assessment.ExportData) [][]any {
	...
	rows := make([][]any, 0, len(data.Contexts)*5)
	for _, c := range data.Contexts {
		...
		for _, spec := range []struct {
			...
			if spec.kind == "vm" || spec.kind == "snapshot" {
