...
// errDeferredCredentialPrompt is returned when automatic work discovers that
// it would need to interrupt the operator for a password. Startup and periodic
// refreshes may reuse an existing session or a stored credential, but they do
// not open an interactive prompt. Selecting or explicitly reloading the
// context starts a foreground load which is allowed to ask.
var errDeferredCredentialPrompt = errors.New("credential entry requires an explicit context load")
